...
export const ViewSavedListDialog: React.FC<Props> = ({ }) => {
    ...
    const viewSavedListDialogOpen = useAppSelector((state) => state.navigation.viewSavedListDialogOpen)


    const handleClose = () => {
        dispatch(navigationSlice.actions.setViewSavedListDialogOpen(false))
    };
    ...
    const selectedListData = useAppSelector((state) => state.navigation.selectedList)
    ...
    React.useEffect(() => {
        ...
        if (selectedListData.buildings) {
            ...
            const fetchBuildingData = async (id: string) => {
                ...
                var tempBuilding: DocumentData = { ...buildingSnap.data(), id: buildingSnap.id }

                return tempBuilding
            }

            var objectEntriesArray = Object.entries(selectedListData.buildings)

            const fetchData = async () => {

                var tempSelectedBuildings = objectEntriesArray.map(async (building) => {
                    const result = await fetchBuildingData(building[0]);
                    //@ts-ignore
                    return { ...result, premises: building[1].premises, images: building[1].images }
    ...
    return (
        <div>
            ...
            <Dialog
                fullWidth={true}
                maxWidth={false}
                closeAfterTransition={true} open={viewSavedListDialogOpen} onClose={handleClose}
                TransitionProps={{
                    onExited: () => {
...
export default ViewSavedListDialog