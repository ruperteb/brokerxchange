...
export const AddContactDialog: React.FC<Props> = ({ landlordId, setSelectedContact }) => {

    const router = useRouter()

    const dispatch = useAppDispatch()
    const addContactDialogOpen = useAppSelector((state) => state.navigation.addContactDialogOpen)


    const handleClose = () => {
        dispatch(navigationSlice.actions.setAddContactDialog(false))
    };
    ...
    const submitContact = async () => {
        ...
        await setDoc(doc(db, "landlords/" + landlordId + "/contacts", contactDetails.name), {
            ...
            .then((result) => {
                ...
                setSelectedContact({
                    ...
                    name: contactDetails.name,
                    email: contactDetails.name,
                    mobile: contactDetails.name,
                    office: contactDetails.name,
                })
                /*  if (result.id) { */
                /*  setSuccess(true) */
                /*  notify() */
    ...
    return (
        <div>
            ...
            <Dialog
                maxWidth="xl"
                closeAfterTransition={true} open={addContactDialogOpen} onClose={handleClose}
                TransitionProps={{
                    onExited: () => {
